...
public enum DeviceMultiLogin {
     ...
     * @return
     */
    public static DeviceMultiLogin getMember(int ordinal) {
        for (int i = 0; i < DeviceMultiLogin.values().length; i++) {
            if (DeviceMultiLogin.values()[i].getLoginMode() == ordinal) {
                return DeviceMultiLogin.values()[i];
            }
        }
        return THREE;
    }

    DeviceMultiLogin(int loginMode, String loginDesc){
